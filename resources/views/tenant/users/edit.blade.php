            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('users.update', $user->id) }}">
                        ...
                        <div class="form-group row">
                            <label for="enterprise_ids" class="col-md-4 col-form-label text-md-right">{{ __('Enterprises') }}</label>

                            <div class="col-md-6">
                                <select class="form-control selectpicker" name="enterprise_ids[]" id="enterprise_ids" multiple>
                                    @foreach($enterprises as $enterprise)
                                    <option value="{{$enterprise->id}}" {{ ( in_array($enterprise->id, $user_enterprise_ids) ) ? 'selected' : '' }}>
                                        {{$enterprise->name}}
                                    </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <select class="form-control selectpicker" name="role_id" id="role_id" required>
                                    ...
                                    <option value="{{$role->id}}" {{ ( in_array($role->id, $user_role_ids) ) ? 'selected' : '' }}>
                                        {{$role->name}}
                                    </option>
                                    @endforeach
                                </select>
